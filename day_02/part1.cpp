...
bool IsGamePossible(std::unordered_map<std::string, int>& map)
{
    return !(map["red"] > kMaxRed || map["green"] > kMaxGreen || map["blue"] > kMaxBlue);
}
...
int main()
{
    ...
    int sum { 0 };
    for (int i = 0; i < color_to_max_mappings.size(); ++i)
        if (IsGamePossible(color_to_max_mappings[i]))
            sum += i + 1;
